...
export const ratingToMovieList = mysqlTable("rating_to_movie_list", {
  ...
  ratingId: int("rating_id").notNull()/* .references(() => rating.id, { onDelete: "cascade", onUpdate: "cascade" }) */,
},
  (table) => {
    return {
...
export const ratingToMovieReview = mysqlTable("rating_to_movie_review", {
  movieReviewId: int("movie_review").notNull()/* .references(() => movieReview.id, { onDelete: "cascade", onUpdate: "cascade" }) */,
  ratingId: int("rating_id").notNull()/* .references(() => rating.id, { onDelete: "cascade", onUpdate: "cascade" }) */,
},
  (table) => {
    return {
...
export const ratingToMovieReviewRelations = relations(ratingToMovieList, ({ one, many }) => ({
