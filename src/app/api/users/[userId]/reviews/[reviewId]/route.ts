...
export async function PATCH(
  ...
  try {
    ...
    if (token && (userIdFromParams === token.id || isAdmin(token))) {
      ...
      // Update the review.
      await db.update(movieReview).set(body).where(eq(movieReview.id, params.reviewId))
  ...
  try {
    ...
    if ((token && (userIdFromParams === token.id || isAdmin(token)))) {
      ...
      if (!resultHeader.rowsAffected) {
        return new Response('This review does not exist or is already deleted!', { status: 404 })
      }

      return new Response(`Deleted review with id: ${params.reviewId}`, { status: 200 })
    }
