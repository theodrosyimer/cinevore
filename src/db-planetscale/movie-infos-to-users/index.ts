...
import {
  ...
  mysqlEnum,
  mysqlTable,
  primaryKey, timestamp, uniqueIndex, varchar
...
export const movieInfosToUser = mysqlTable("movie_infos_to_user", {
  ...
  (table) => {
    return {
      ...
      compositeKeyIndex: uniqueIndex("composite_key").on(table.movieId, table.userId),
...
export const movieInfosToUserRelations = relations(movieInfosToUser, ({ one }) => ({
  movie: one(movie, { fields: [movieInfosToUser.movieId], references: [movie.tmdbId], relationName: 'movie' }),
  user: one(user, { fields: [movieInfosToUser.userId], references: [user.id], relationName: 'user' }),
}))
