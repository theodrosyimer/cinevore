...
export const watchlistToMovies = mysqlTable('watchlist_to_movies', {
  watchlistId: int('watchlist_id').notNull()/* .references(() => watchlist.id, { onDelete: "cascade", onUpdate: "cascade" }) */,
...
export const watchlistToMoviesRelations = relations(watchlistToMovies, ({ one, }) => ({
  watchlist: one(watchlist, { fields: [watchlistToMovies.watchlistId], references: [watchlist.id] }),
  movie: one(movie, { fields: [watchlistToMovies.movieId], references: [movie.tmdbId] }),
}))
