// TODO: need to change parameters types from `./seed.ts`
...
const mySqlType = await import("@/db/schema/mysql2/index.js")
...
export async function addMovies(tx: MySqlTransactionSeed) {
  await Promise.all(
...
export async function addLists(tx: MySqlTransactionSeed) {
  await Promise.all(
...
export async function addMovieLists(tx: MySqlTransactionSeed) {
  await Promise.all(
...
export async function addMovieReviews(tx: MySqlTransactionSeed) {
  await Promise.all(
...
export async function addComments(tx: MySqlTransactionSeed) {
  await Promise.all(
...
export async function addLikes(tx: MySqlTransactionSeed) {
  await Promise.all(
...
export async function addFollowers(tx: MySqlTransactionSeed) {
  await Promise.all(
...
export async function addWatchlists(tx: MySqlTransactionSeed) {
  await Promise.all(
